...
#$all=Import-Excel 'C:\temp\Coastal - Email Choice.xlsx' -WorksheetName 'Coastal Vet Hospital'
$all=import-excel 'C:\Temp\Riverport\Riverport - Email Choice.xlsx'


$path='C:\Temp\todolist\Riverport-mailusers.xlsx'


$temp=$all | Where-Object {$_.mailtype -eq 'mailbox' -and $_.preference -like '*mail box*'} | select -First 1
$tempemail=$temp.Companymail
$tmpuser=get-aduser -filter "mail -eq '$tempemail'"#{mail -eq $tempemail} 
    ...
    if(  $one.preference -like '*Mail User*'){
        ...
        if ($result -eq $null){
             ...
             if($exist){ Write-Host "Mailbox $companyemail still exist.. please wait for 10 min and try again.." }
                ...
                $flag=get-mailuser $name -erroraction Silentlycontinue 
