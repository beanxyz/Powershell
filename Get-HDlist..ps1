        ...
        FOREACH ($Member in $GroupMembers)
        {
            TRY
            {
                ...
                SWITCH ($Member.RecipientType)
                {
                    ...
                    "UserMailbox" {
                        # Member's type is User, let's just output the $gg=get-group $group; $dn=$gg.distinguishedname;

                        #$group="Vetcare Unitec - Hospital Director"
                      $gg=get-group $group; $dn=$gg.distinguishedname;
                       $area=Get-Group -Filter "Members -eq '$dn'" 

                       foreach($one in $area){
                       
                        if($one.DisplayName -like "*directors*"){
...
Get-DistributionGroupMemberRecursive  "Hospital Directors - ALL" | tee -Variable result
...
$temp=[pscustomobject]@{Country=$newcountry;Area=$newarea;Clinic=$newclinic;Firstname=$line.FirstName;Lastname=$line.LastName; Name=$line.Name; Email=$line.PrimarySmtpAddress; Code=$code;flag=$flag}
...
$r2 | Export-Excel 'C:\temp\HD list.xlsx'